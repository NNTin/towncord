...
export class TerrainRenderer {
  ...
  private resolveFrameForCurrentPhase(
    ...
    const durationsMs = this.animationClock.getPhaseDurationsForBaseFrame(
      baseFrame,
      variants.length,
    );
  ...
  private renderTilesToRT(
    ...
    chunkStartX: number,
    chunkStartY: number,
    resolveFrame: (baseFrame: string, tile: TerrainRenderTile) => string,
  ): void {
    if (tiles.length === 0) {
      rt.clear();
      return;
    ...
    for (const tile of tiles) {
      ...
      const frame = resolveFrame(tile.frame, tile);
