...
export class WorldSceneTerrainController {
  ...
  public syncPreviewAtScreen(screenX: number, screenY: number): void {
    if (!this.activeTerrainTool || !this.host.getTerrainSystem() || !this.terrainBrushPreview) {
      this.setTerrainBrushPreviewVisible(false);
      this.hideTerrainBrushRenderPreview();
      return;
    }
    ...
    const grid = terrainSystem.getGameplayGrid();
    ...
    const previewTiles = terrainSystem.previewPaintAtWorld(
      {
        type: "terrain",
  ...
  public paintAtScreen(screenX: number, screenY: number): void {
    const terrainSystem = this.host.getTerrainSystem();
    if (!this.activeTerrainTool || !terrainSystem) {
      return;
    }

    const worldPoint = this.host.scene.cameras.main.getWorldPoint(screenX, screenY);
    const cell = terrainSystem.getGameplayGrid().worldToCell(worldPoint.x, worldPoint.y);
    if (
      !cell ||
  ...
  private queueTerrainDropAtWorld(
    ...
    const cell = terrainSystem.getGameplayGrid().worldToCell(worldX, worldY);
    if (!cell || this.isTerrainCellOccupied(cell)) {
      return;
    }

    terrainSystem.queueDrop(payload, worldX, worldY);
  }
