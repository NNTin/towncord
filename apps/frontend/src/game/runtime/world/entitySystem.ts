...
export class EntitySystem {
  ...
  update(delta: number, directInput: MovementInput): void {
    ...
    const hasDirectMovement =
      directInput.moveX !== 0 || directInput.moveY !== 0;
    this.directInputIdleMs = hasDirectMovement
      ? 0
      : this.directInputIdleMs + delta;
    ...
    for (const entity of this.entities) {
      ...
      const dirChanged =
        entity.state !== "idle" && entity.facing !== prevFacing;
