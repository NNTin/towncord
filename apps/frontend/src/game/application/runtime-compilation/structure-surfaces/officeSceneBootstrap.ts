...
const FURNITURE_RENDER_ASSET_BY_ID = new Map(
...
function mapFurnitureEntry(
  ...
  const renderAsset = toFurnitureRenderAsset(entry.type);
...
function toFurnitureRenderAsset(assetId: string) {
  const paletteItem = FURNITURE_RENDER_ASSET_BY_ID.get(assetId);
  if (!paletteItem) {
    return null;
  }
...
function isFurnitureRecord(value: unknown): value is OfficeSceneFurniture {
  return (
    ...
    (!("renderAsset" in (value as OfficeSceneFurniture)) ||
      (value as OfficeSceneFurniture).renderAsset == null ||
      isFurnitureRenderAssetRecord((value as OfficeSceneFurniture).renderAsset))
  );
}

function isFurnitureRenderAssetRecord(value: unknown): boolean {
  ...
  return (
    ...
    Number.isFinite(candidate.atlasFrame.x) &&
    Number.isFinite(candidate.atlasFrame.y) &&
    Number.isFinite(candidate.atlasFrame.w) &&
    Number.isFinite(candidate.atlasFrame.h)
  );
}
